...
export default {
  ...
  methods: {
    ...
    submitForm() {
      if (!this.isEdit) {
        console.log("member id :", this.$store.getters.myMember_id);
        if (confirm("คุณต้องการบันทึกการเพิ่มข้อมูลหรือไม่?")) {
          console.log("บันทึกการเพิ่มข้อมูล", this.plan);
          const newPlan = {
    ...
    deleteUser(plan_id, plan_title) {
      if (confirm("คุณต้องการลบการพัฒนา [" + plan_title + "] หรือไม่ ?")) {
        var self = this;
        axios
          .post("http://localhost/ICPScoreCard/api-plan.php", {
            action: "delete",
    ...
    getUpdate() {
      console.log(" แสดงข้อมูลการพัฒนาตนเอง id : ", this.plan.member_id);
      var self = this;
      axios
        .post("http://localhost/ICPScoreCard/api-plan.php", {
          action: "get_plan_by_qa_plan_career_id",
          member_id: this.plan.member_id,
        })
        .then(function (res) {
