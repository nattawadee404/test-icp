...
export default {
  ...
  methods: {
    ...
    submitForm() {
      if (!this.isEdit) {
        ...
        if (confirm("คุณต้องการเพิ่มข้อมูลการประเมินตนเองหรือไม่ ?")) {
          console.log("บันทึกข้อมูล", this.selfAssessment);
          const newSelfAssessment = {
            self_assessment_id: this.self_assessment,
    ...
    getUpdate() {
      console.log(
        " แสดงข้อมูลการประเมินตนเอง id : ",
        this.selfAssessment.member_id
      );
      var self = this;
      axios
        .post("http://localhost/ICPScoreCard/api-self-assessment.php", {
          action: "getAll",
          member_id: this.selfAssessment.member_id,
        })
        .then(function (res) {
